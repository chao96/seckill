...
* [Java商城秒杀系统设计与实战](#java商城秒杀系统设计与实战)
  * [一、项目整体简介](#一项目整体简介)
    * [1.核心技术列表](#1核心技术列表)
    * [2.项目业务功能介绍](#2项目业务功能介绍)
    * [3.项目学习收益](#3项目学习收益)
  * [二、项目搭建](#二项目搭建)
    * [1.项目相关环境](#1项目相关环境)
    * [2.项目准备](#2项目准备)
      * [1）数据库相关表创建](#1数据库相关表创建)
      * [2)SpringBoot搭建多模块项目](#2springboot搭建多模块项目)
    * [3）配置文件](#3配置文件)
      * [4）启动项目](#4启动项目)
  * [三、秒杀业务实现](#三秒杀业务实现)
    * [1.基础工作](#1基础工作)
      * [1）创建通用类](#1创建通用类)
      * [2）创建BaseController类测试接口](#2创建basecontroller类测试接口)
      * [3）Mybatis逆向工程](#3mybatis逆向工程)
    * [2.业务代码编写](#2业务代码编写)
      * [1）商品列表展示](#1商品列表展示)
      * [2) 订单编号生成](#2-订单编号生成)
      * [3）秒杀商品核心逻辑V1](#3秒杀商品核心逻辑v1)
      * [4）整合RabbitMQ实现消息异步发送](#4整合rabbitmq实现消息异步发送)
      * [5）邮件服务发送通知消息](#5邮件服务发送通知消息)
      * [6）死信队列失效超时未支付的订单](#6死信队列失效超时未支付的订单)
      * [7）定时任务提高安全性--处理死信队列未工作情况](#7定时任务提高安全性--处理死信队列未工作情况)
      * [8）Jmeter高并发压力测试](#8jmeter高并发压力测试)
      * [9）问题分析](#9问题分析)
  * [四、秒杀逻辑优化](#四秒杀逻辑优化)
...
**辅助方案一**：中间件（RabbitMQ、Redis）服务做集群部署，提高高可用和稳定性。
